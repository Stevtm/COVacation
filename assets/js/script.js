...
function infoLookup(cityName) {
	...
	.then(response => {
		if(response.ok){
			response.json()
			.then(data => {
				// console.log(`${cityName}		${data.results[0].components.country}		${data.results[0].components['ISO_3166-1_alpha-3']}`);
				// showCovidInfo()

				// console.log(data); // data from opencage api

				var cityLat = data.results[0].annotations.DMS.lat;
				var cityLon = data.results[0].annotations.DMS.lng;
				...
				.then(response => {
					...
					.then(covidData => {
						...
						// get weather info from this fetch
						// use onecall api instead with lat lon from opencage api data
						fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&APPID=3e8aa64128a8b382a871af127be1e2d0&units=imperial&exclude=current,minutely,hourly`)
						.then(response => {
							response.json()
							.then(weatherData => {
